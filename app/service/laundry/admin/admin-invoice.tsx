...
export default function ShopInvoiceScreen() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        const ref = doc(db, "shop_orders", orderId);
        const snap = await getDoc(ref);
        if (snap.exists()) {
          setOrder({ id: snap.id, ...snap.data() });
        } else {
          Alert.alert("Error", "Shop order not found.");
        }
      } catch (err) {
        Alert.alert("Error", "Failed to fetch shop order details.");
      } finally {
        setLoading(false);
      }
    };
    ...
    const html = `
      <html>
        ...
        <body>
          ...
          <div class="footer">
            <p>For support, call: <strong>08140002708</strong></p>
